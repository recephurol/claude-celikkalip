...
import { getReferences, addReference, updateReference, deleteReference, fileToBase64, Reference } from '../../lib/store';

export default function AdminReferences() {
  const [references, setReferences] = useState<Reference[]>(getReferences());
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (editingRef) {
      updateReference(editingRef.id, formData);
    } else {
                ...
                <div className="flex items-center gap-3">
                  ...
                  {formData.logoUrl && (
                    <div className="h-12 px-3 bg-white border border-[var(--color-border)] rounded-lg flex items-center">
                      <img src={formData.logoUrl} alt="Logo" className="max-h-10 w-auto object-contain" />
                    </div>
                  )}
                </div>
              </div>
          ...
          {references.map((ref) => (
            <div key={ref.id} className="bg-white rounded-xl p-5 shadow-sm border border-[var(--color-border)]">
              ...
              <div className="bg-[var(--color-bg-alt)] rounded-lg p-4 flex items-center justify-center h-24">
                <img src={ref.logoUrl} alt={ref.name} className="max-h-14 w-auto object-contain" />
              </div>
