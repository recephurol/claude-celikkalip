...
export default function AdminProductsPage() {
  ...
  function resetForm() { setFormData({ title: "", description: "", category: "", order: 0 }); setEditingProduct(null); setShowForm(false); }

  function startEdit(p: Product) { setFormData({ title: p.title, description: p.description || "", category: p.category || "", order: p.order }); setEditingProduct(p); setShowForm(true); }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (editingProduct) {
      await fetch("/api/products", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editingProduct.id, ...formData }) });
    } else {
  ...
  async function handleDelete(id: number) { if (!confirm("Bu urunu silmek istediginize emin misiniz?")) return; await fetch(`/api/products?id=${id}`, { method: "DELETE" }); fetchProducts(); }

  async function handleImageUpload(productId: number, files: FileList) {
    setUploading(true);
    for (const file of Array.from(files)) { const fd = new FormData(); fd.append("file", file); fd.append("productId", String(productId)); fd.append("alt", file.name); await fetch("/api/upload", { method: "POST", body: fd }); }
    setUploading(false); fetchProducts();
  }

  async function handleImageDelete(imageId: number) { await fetch(`/api/upload?id=${imageId}`, { method: "DELETE" }); fetchProducts(); }
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--color-border)] mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium mb-1">Baslik *</label><input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required className="w-full px-4 py-2.5 rounded-lg border border-[var(--color-border)] outline-none focus:ring-2 focus:ring-[var(--color-primary)]" /></div>
              <div><label className="block text-sm font-medium mb-1">Kategori</label><input type="text" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-[var(--color-border)] outline-none focus:ring-2 focus:ring-[var(--color-primary)]" /></div>
            </div>
            <div><label className="block text-sm font-medium mb-1">Aciklama</label><textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-4 py-2.5 rounded-lg border border-[var(--color-border)] outline-none focus:ring-2 focus:ring-[var(--color-primary)] resize-none" /></div>
          ...
          <div key={product.id} className="bg-white rounded-xl p-6 shadow-sm border border-[var(--color-border)]">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(product)} className="p-2 text-[var(--color-primary)] hover:bg-[var(--color-bg-alt)] rounded-lg"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
